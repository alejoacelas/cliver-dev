...
export interface CheckSchedulerOptions {
  ...
  /** Optional hook that runs after decision computation, before emitting pipeline_complete. */
  postDecision?: (
    decision: Decision,
...
export class CheckScheduler {
  ...
  private readonly postDecision?: CheckSchedulerOptions["postDecision"];
  ...
  constructor(options: CheckSchedulerOptions) {
    ...
    this.postDecision = options.postDecision;
  ...
  private async checkCompletion(): Promise<void> {
    ...
    if (allDone && this.status !== "completed") {
      let decision = this.aggregator.computeDecision(this.outcomes);
      if (this.postDecision) {
        decision = await this.postDecision(decision, this.outcomes, this.fieldValues);
      }
